/**
 * ============================================
 * 🚛 Truck List Component - نظام إدهام
 * Edham Logistics - Truck List
 * ============================================
 */

import React, { useState, useEffect } from 'react';
import api from '../../services/api';
import { Loader, AlertCircle, Truck, Search, Filter, Plus } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-white">الشاحنات</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-white text-sm">
            <thead className="bg-gray-700">
              <tr>
                <th className="px-4 py-3 text-right">رقم الشاحنة</th>
                <th className="px-4 py-3 text-right">اللوحة</th>
                <th className="px-4 py-3 text-right">النوع</th>
                <th className="px-4 py-3 text-right">السعة</th>
                <th className="px-4 py-3 text-right">السائق الحالي</th>
                <th className="px-4 py-3 text-right">الكيلومترات</th>
              ...
              {filteredTrucks.length === 0 ? (
                ...
                filteredTrucks.map((truck) => (
                  <tr key={truck._id} className="border-t border-gray-700 hover:bg-gray-700">
                    <td className="px-4 py-3 font-semibold">{truck.truckNumber}</td>
                    <td className="px-4 py-3">{truck.plateNumber}</td>
                    <td className="px-4 py-3">
                      {truck.type === 'refrigerated' ? 'مبردة' : 'عادية'}
                    </td>
                    <td className="px-4 py-3">{truck.capacity} طن</td>
                    <td className="px-4 py-3">{truck.currentDriver?.name || 'غير محدد'}</td>
                    <td className="px-4 py-3">{truck.currentKilometers?.toLocaleString() || 0} كم</td>
